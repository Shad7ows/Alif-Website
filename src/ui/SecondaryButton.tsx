...
export function SecondaryButton({
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-24 h-7 bg-[#247BFF] blur-md rounded-full pointer-events-none" />
      <button
        type={type}
        onClick={onClick}
        className={`
          px-3 rounded-2xl h-12.5 w-35 font-medium transition-all cursor-pointer text-[16px] z-10
          relative border border-[#247BFF] text-[#eeeeee] bg-[#247BFF]/5 backdrop-blur-md
        `.trim()}
      >
        {children}
      </button>
